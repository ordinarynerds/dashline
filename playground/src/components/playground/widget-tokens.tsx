import { ICONS, colorOf, widgetParts, type Item, type Scenario } from '@/lib/dashline'
...
// scenario (terminal preview only) swaps in live-changing values for playback.
export function WidgetTokens({ id, item, theme = '', icons = false, scenario }: WidgetTokensProps) {
  const resolved: Item = item ?? { widget: id }
  const parts = widgetParts(resolved, scenario)
  ...
  return (
    <span className="whitespace-pre" style={attrs}>
      {glyph ? <span style={{ color: colorOf('dim', theme) }}>{glyph} </span> : null}
      {parts.map(([text, c], i) => {
